...
doccm <- function(x, y) {
	e.x <- BestEmbed(x)
	e.y <- BestEmbed(y)
	ccm.raw.x <- ccm(cbind(x, y), E = 3, lib_sizes = seq(10, 80, by=10), silent = TRUE, num_samples = 100, random_libs = TRUE, replace = TRUE, RNGseed = 123)
	ccm.raw.y <- ccm(cbind(y, x), E = 3, lib_sizes = seq(10, 80, by=10), silent = TRUE, num_samples = 100, random_libs = TRUE, replace = TRUE, RNGseed = 123)
...
df <- sardine_anchovy_sst
df <- df[, c(2,5)]
d <- df
names(d) <- c("x", "y")
# Anchovy is "x", and np_sst is "y"
